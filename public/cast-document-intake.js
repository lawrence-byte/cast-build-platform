...
(function(){
  ...
  const MODULE_FOLDERS={'Documents':'general','Contracts':'contracts','Financials':'financials','Field':'field','Drawings':'drawings','RFIs':'rfis','Submittals':'submittals','Change Orders':'change_orders','Pay Applications':'pay_applications','Invoices':'invoices','Insurance':'insurance','Permits':'permits','Meeting Minutes':'meeting_minutes','Closeout':'closeout','Uncategorized':'uncategorized'};
  ...
  function classify(file){
    ...
    const folder=`/server_storage/projects/${project.id}/documents/${MODULE_FOLDERS[module]||'uncategorized'}`;
    const linked=[];
    ...
    return {projectId:project.id,projectName:project.name,module,documentType:module==='RFIs'?'RFI / response':module==='Submittals'?'Submittal package / response':module,confidenceScore:confidence,suggestedFolderPath:folder,suggestedRecordLinks:linked.map(label=>({label,module,confidence:.85,duplicateWarning:/RFI|Submittal/.test(label)})),extractedMetadata:{fileExtension:e,folderContext:'',textExtracted:false,ocrUsed:false,dates:[],documentTitle:name.replace(/\.[^.]+$/,''),documentHeader:'',documentFooter:'',signaturesDetected:/signed|executed/i.test(name),emailAddresses:[],costValues:[],keywords:[module]},requiresHumanReview:confidence<.92||linked.length>0,reasoningSummary:reason,route,status:'Ready for confirmation'};
  }
  function ensureUi(){
  ...
  function render(){
    ...
    list.innerHTML=items.length?items.map((it,i)=>`<div class="cast-intake-file ${selected===it.id?'is-selected':''}"><button data-intake-select="${esc(it.id)}"><strong>${esc(it.name)}</strong><span class="cast-intake-muted">${esc(it.classification.module)} · ${esc(it.classification.projectName)} · ${Math.round(it.classification.confidenceScore*100)}% confidence</span><div class="cast-intake-progress"><span style="width:${it.progress||0}%"></span></div><span class="cast-intake-pill">${esc(it.status)}</span></button></div>`).join(''):'<p class="cast-intake-muted">No files queued yet.</p>';
    ...
    detail.innerHTML=`<div class="cast-intake-preview" data-preview>${it.preview?`<img src="${esc(it.preview)}" alt="Preview of ${esc(it.name)}">`:`<span class="cast-intake-muted">Preview unavailable for this file type</span>`}</div><p><strong>${esc(it.name)}</strong><br><span class="cast-intake-muted">${esc(it.sizeLabel)} · ${esc(it.type||'unknown type')}</span></p><p><span class="cast-intake-pill">${esc(c.module)}</span><span class="cast-intake-pill">${esc(c.projectName)}</span><span class="cast-intake-pill">${Math.round(c.confidenceScore*100)}% confidence</span></p><form class="cast-intake-form" data-intake-confirm-form><label>Project<select name="projectId">${PROJECTS.map(p=>`<option value="${p.id}" ${p.id===c.projectId?'selected':''}>${esc(p.name)}</option>`).join('')}</select></label><label>Module<select name="module">${MODULES.map(m=>`<option ${m===c.module?'selected':''}>${esc(m)}</option>`).join('')}</select></label><label class="wide">Suggested Folder<input name="folder" value="${esc(c.suggestedFolderPath)}"></label><label class="wide">Linked Records<input name="linkedRecords" value="${esc((c.suggestedRecordLinks||[]).map(x=>x.label||x).join(', '))}" placeholder="RFI 12, Submittal 034, Drawing A2.01"></label><label class="wide">Admin Override / Filing Note<textarea name="note" rows="3" placeholder="Reason for override, access restrictions, routing note"></textarea></label><div class="wide"><button class="cast-intake-btn" type="submit">Confirm and File</button> <button class="cast-intake-btn secondary" type="button" data-intake-remove="${esc(it.id)}">Remove</button></div></form><p class="cast-intake-muted">Reason: ${esc(c.reasoningSummary)}. Final filing requires user confirmation and admin override is captured before server storage.</p>`;
  }
