...
(async()=>{[budget,audit,revisions,forecast]=await Promise.all([fetchJson('/safe-data/projects/golden-hill/procore-information/budget/budget-summary.json'),fetchJson('/safe-data/projects/golden-hill/procore-information/budget/budget-audit.json'),fetchJson('/safe-data/projects/golden-hill/procore-information/budget-changes/budget-revisions-register.json').catch(()=>null),fetchJson('/safe-data/projects/golden-hill/procore-information/budget-forecast/forecast-summary.json').catch(()=>null)]);buildExceptions();renderSummary();renderActions();renderAudit();['q','flagFilter','severityFilter'].forEach(id=>document.querySelector('#'+id).addEventListener('input',renderRows));renderRows();})().catch(e=>document.body.insertAdjacentHTML('afterbegin',`<div class="note"><strong>Budget exception metadata unavailable:</strong> Refresh the page or check the latest deployment.</div>`));
