...
function renderCandidates(){
  ...
  const rows=candidates.filter(c=>{const projectPath=c.projectPath||c.relativeToAlum||c.path||'';return (!cat||c.category===cat)&&(!mode||(mode==='yes'?c.ocrCandidate:c.nativeTextCharsFirst2===0))&&(!q||[c.fileName,c.category,c.subcategory,projectPath].join(' ').toLowerCase().includes(q));}).slice(0,150);
  document.querySelector('#candidate-count').textContent=`${rows.length} shown`;
  document.querySelector('#rows').innerHTML=rows.map(c=>{const projectPath=c.projectPath||c.relativeToAlum||c.path||'';const folder=projectPath.split('/').slice(0,2).join(' / ');return `<tr><td><strong>${esc(c.fileName)}</strong>${c.ocrCandidate?'<br><span class="pill pending">OCR candidate</span>':''}</td><td>${esc(c.category)}<br><span class="caption">${esc(c.subcategory)}</span></td><td>${esc(c.pages)}</td><td>${esc(c.nativeTextCharsFirst2)} chars</td><td>${esc(c.priority)}</td><td><span class="caption">${esc(folder||'Private project source')}</span></td></tr>`}).join('');
}
function render(){
  ...
  document.querySelector('#sampleCards').innerHTML=samples.map(s=>{const projectPath=s.projectPath||s.relativeToAlum||s.path||'';const folder=projectPath.split('/').slice(0,2).join(' / ');return `<article class="card span-6"><header><h2>${esc(s.fileName)}</h2><span class="pill approved">${esc(s.extractionStatus.replaceAll('_',' '))}</span></header><div class="body"><p><strong>${esc(s.category)}</strong> · ${esc(s.pagesProcessed)} page(s) OCR’d · ${esc(s.ocrTextChars)} chars recovered</p><p class="caption">${esc(folder||'Private project source')}</p><div class="note"><strong>Preview:</strong> ${esc(s.textPreview)}</div>${s.extractedTerms?.length?`<p class="caption"><strong>Terms:</strong> ${esc(s.extractedTerms.join(', '))}</p>`:''}${s.reviewFlags?.length?`<ul>${s.reviewFlags.map(f=>`<li>${esc(f)}</li>`).join('')}</ul>`:''}</div></article>`}).join('');
