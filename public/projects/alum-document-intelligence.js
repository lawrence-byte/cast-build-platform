...
function renderCandidates(){
  ...
  const rows=candidates.filter(c=>(!cat||c.category===cat)&&(!mode||(mode==='yes'?c.ocrCandidate:c.nativeTextCharsFirst2===0))&&(!q||[c.fileName,c.category,c.subcategory,c.relativeToAlum].join(' ').toLowerCase().includes(q))).slice(0,150);
  document.querySelector('#candidate-count').textContent=`${rows.length} shown`;
  document.querySelector('#rows').innerHTML=rows.map(c=>`<tr><td><strong>${esc(c.fileName)}</strong>${c.ocrCandidate?'<br><span class="pill pending">OCR candidate</span>':''}</td><td>${esc(c.category)}<br><span class="caption">${esc(c.subcategory)}</span></td><td>${esc(c.pages)}</td><td>${esc(c.nativeTextCharsFirst2)} chars</td><td>${esc(c.priority)}</td><td><span class="caption">${esc(c.relativeToAlum)}</span></td></tr>`).join('');
}
function render(){
