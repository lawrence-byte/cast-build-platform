...
const apiContract = fs.readFileSync(path.join(root, 'api/_lib/project-controls-api.js'), 'utf8');
for (const requiredApiSignal of ['AUTH_REQUIRED', 'BACKEND_NOT_CONFIGURED', 'HANDLER_NOT_IMPLEMENTED', 'ProjectAccess', 'DistributionRecipients']) {
  if (!apiContract.includes(requiredApiSignal)) fail(`Project-controls API contract missing fail-closed signal: ${requiredApiSignal}`);
}
const vercelApiRewrite = JSON.parse(fs.readFileSync(path.join(root, 'vercel.json'), 'utf8')).rewrites.find((row) => row.source === '/api/:path*');
if (vercelApiRewrite) fail('Vercel /api/* must not rewrite to a static placeholder; API functions must handle protected routes fail-closed.');
